...
2. **Debug Menu** (hidden by default) — All canva-mcp framework links (7 stages, agent files, tools). Only shown when the user clicks the **debug button** at the **bottom-right corner** of the page.
...
**Goal:** Create a template project that can be used by other projects at start — `canva-mcp` v0.9
...
|-------------|-------------------------------|------------------|
| `{{PROJECT_NAME}}` | `canva-mcp` | `README.md`, `index.html` (title, hero), `2_Environment/supabase/config.toml` (project id) |
| `{{GITHUB_USER}}` | `rifaterdemsahin` | `README.md`, `index.html` (GitHub link), renderer local fallback |
| `{{REPO_NAME}}` | `canva-mcp` | `README.md` Pages URL, `sitemap.xml`, `robots.txt` |
| `{{PAGES_URL}}` | `https://rifaterdemsahin.github.io/canva-mcp/` | `README.md`, `sitemap.xml`, `robots.txt` |
