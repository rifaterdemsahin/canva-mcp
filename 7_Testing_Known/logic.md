...
| 7 | A local API can accept `{ title, voiceover }` and "do the MCP call in the background" to produce a Canva presentation | Objective 8: Voiceover-to-Presentation API (SPEC-018) | TSK-034…039: confirmed no REST equivalent to `generate-design-structured` exists (only the MCP-connector context can call it); built a deterministic voiceover→outline parser + job queue (`5_Symbols/presentation-api/`) instead of a headless generator; live run: `POST` → job `46593a64-...` → real MCP chain run interactively → `complete-job` → design `DAHPMbFhUJE` (11 pages) verified via `get-design` | "Background" cannot mean a fully headless script calling Canva's AI generation — no credential/endpoint exists for that. User confirmed the correct model: fast/deterministic parts are real code (parse, queue, store); the one step requiring AI design generation is agent-run interactively, not faked as automation | API + parser + job store + `complete-job` CLI all real, working, tested (11/11 unit tests, 3 real bugs found and fixed by the tests: quote-matching false-positive, sentence-splitter silently dropping text around embedded periods, a `RegExp.lastIndex`-reset bug). Live end-to-end run produced a real design | ✅ Confirmed |
